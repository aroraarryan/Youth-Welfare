...
export default function KhelMaidaanPage() {
  ...
  return (
    <>
      <PageHero
        hindiTitle="खेल मैदान"
        title="Khel Maidaan — Infrastructure"
        subtitle="Public playgrounds and sports fields across Uttarakhand districts"
              ...
              <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                  ...
                  <h2 className="text-3xl lg:text-4xl font-black text-[#1e293b] leading-tight">
                    {selectedDistrict?.name} <span className="text-[#1e3a8a]/40 italic">Maidaans</span>
                  </h2>
                </div>
                ...
                <div className="flex items-center gap-2 px-5 py-2.5 bg-blue-50 text-[#1e3a8a] rounded-full text-[10px] font-black uppercase tracking-widest border border-blue-100 shadow-sm">
                   ...
                   {maidaans.length} Result{maidaans.length !== 1 ? 's' : ''} Found
                </div>
              </div>
              ...
              {loading ? (
                ...
                <div className="bg-white rounded-[3rem] p-24 text-center border border-gray-100 shadow-sm max-w-2xl mx-auto">
                  <div className="w-24 h-24 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-10 border border-gray-100 shadow-inner">
                    <i className="fas fa-futbol text-4xl text-gray-200" />
                  </div>
                  <h3 className="text-2xl font-black text-[#1e293b] mb-4">No Maidaans Registered</h3>
                  <p className="text-[#64748b] font-medium leading-relaxed max-w-sm mx-auto">
                    We currently don't have any khel maidaans registered for {selectedDistrict?.name}. Please check back later.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              ...
              <div className="bg-white/40 backdrop-blur-sm rounded-[3rem] p-16 lg:p-24 text-center border border-white shadow-xl overflow-hidden relative">
                <div className="relative z-10 max-w-2xl mx-auto">
                  <div className="w-32 h-32 bg-white rounded-[2.5rem] flex items-center justify-center mx-auto mb-12 shadow-[0_20px_40px_rgba(0,0,0,0.05)] border border-gray-50">
                    <i className="fas fa-futbol text-5xl text-[#1e3a8a]/30 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500" />
                  </div>
                  ...
                  <p className="text-lg lg:text-xl text-[#64748b] font-medium leading-relaxed">
                    Select a district from the selector above to unlock the full directory of khel maidaans and playgrounds in your region.
                  </p>
      ...
      <InfraDetailModal 
        isOpen={!!selectedMaidaan}
        onClose={() => setSelectedMaidaan(null)}
        data={selectedMaidaan ? {
          name: selectedMaidaan.name,
          location: selectedMaidaan.location,
          districtName: selectedMaidaan.district.name,
          facilities: selectedMaidaan.facilities,
          pocName: selectedMaidaan.pocName,
          pocPhone: selectedMaidaan.pocPhone,
          pocEmail: selectedMaidaan.pocEmail,
          imageUrls: selectedMaidaan.imageUrls,
          latitude: selectedMaidaan.latitude,
          longitude: selectedMaidaan.longitude
        } : null}
      />
    </>
  );
