...
import { useFixtureEvents } from '@/hooks/useOfficerCmTrophyFixtures';
...
export default function OfficerCmTrophyFixturesPage() {
  ...
  return (
    <div className="p-6">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <Link
          href="/officer/cm-trophy/fixtures/new"
      ...
      <p className="text-xs text-gray-400 mb-4">
        Vidhan Sabha-level fixtures only — entrants are that Vidhan Sabha&apos;s Nyay Panchayats.
      </p>
              ...
              {data.data.map((ev) => (
                <tr key={ev.id} className="hover:bg-gray-50 cursor-pointer">
                  <td className="px-4 py-3">
                    <Link href={`/officer/cm-trophy/fixtures/${ev.id}`} className="text-[#1e3a8a] font-semibold hover:underline">
                      {ev.sportName}
                    </Link>
                  </td>
