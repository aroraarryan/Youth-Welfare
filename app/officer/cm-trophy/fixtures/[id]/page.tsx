...
function TeamsTab({ event }: { event: import('@/lib/api/adminCmTrophyFixturesApi').FixtureEventDetail }) {
  ...
  const { data: playerPoolRes } = useEntrantPool(
    isPlayerEntrant
      ? { entrantType: 'PLAYER', sportId: event.sportId, ageCategory: event.ageCategory, gender: event.gender ?? undefined, event: event.event ?? undefined, vidhanSabhaId: event.vidhanSabhaId ?? undefined }
      : null
  );
