...
import { AttendanceRow, CmTrophyAttendanceListParams } from '@/lib/api/officerCmTrophyApi';
...
export default function OfficerAttendancePage() {
  // Same page as the admin one. The server does all the scoping: block officers only ever get
  // Nyay Panchayat-level players from their block (so the level filter is hidden for them);
  // district officers get every level in their district.
  ...
  const { data, isLoading, isError, error } = useOfficerAttendanceList(filters);

  // Wider fetch (same filters, minus event) just to build the Event dropdown's option list.
  const { data: eventOptionsData } = useOfficerAttendanceList({ ...filters, event: undefined, page: undefined, limit: 200 });
    ...
    <div className="p-6">
      <div className="mb-5">
        ...
        <p className="text-xs text-gray-400 mt-0.5">
          Check in {isDistrictOfficer ? 'players from your district' : 'Nyay Panchayat-level players'} on event day — mark write-once, cannot be undone.
        </p>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#115e59] text-left">
              ...
              {getPageNumbers(page, Math.ceil(data.total / data.limit)).map((p, i) =>
                p === '...' ? (
                  ...
                  <button
                    ...
                    className={`px-3 py-2 border-r border-gray-200 font-medium ${p === page ? 'bg-[#115e59] text-white' : 'text-gray-700 hover:bg-gray-50'}`}
                  >
                    {p}
                  </button>
                )
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-sm w-full p-6">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={bulkMarkPresent.isPending}
                className="px-4 py-2 text-sm font-medium text-white bg-[#115e59] hover:bg-[#0f766e] rounded-md disabled:opacity-50"
              >
