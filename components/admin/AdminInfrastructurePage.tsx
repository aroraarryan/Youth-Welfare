...
/** Synthetic admin profile so existing InfrastructureForm works in SUPER_ADMIN mode */
const ADMIN_PROFILE = {
  id: 0,
  name: 'Admin',
  email: '',
  ...
  viPhotoUrl: null,
  numBlocks: null,
  numNyayaPanchayats: null,
  numGramPanchayats: null,
  isActive: true,
  createdAt: '',
  lastLogin: null,
};
...
export default function AdminInfrastructurePage({ type, title, newLabel, icon }: Props) {
  ...
  const { data, isLoading, isError, error, refetch } = useAdminInfra(
    type,
    filterDistrictId || undefined,
    page
  );
  const createMutation = useAdminCreateInfra(type);
  const updateMutation = useAdminUpdateInfra(type);
  const deleteMutation = useAdminDeleteInfra(type);
    ...
    <div className="p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">All Districts · Admin View</p>
        </div>
        <button
        ...
        >
          <i className="fas fa-plus" /> {newLabel}
        </button>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4 flex flex-wrap gap-4 items-center shadow-sm">
        ...
        {filterDistrictId && (
          <button
            onClick={() => { setFilterDistrictId(''); setPage(1); }}
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-gray-900 font-medium max-w-[180px] truncate">{item.name}</td>
                    <td className="px-6 py-4 text-gray-500 text-[13px] max-w-[160px] truncate">{item.location || '—'}</td>
                    <td className="px-6 py-4 text-gray-500 text-[13px]">
                      {item.pocName || '—'}
                      {item.pocPhone && (
                        <span className="block text-[11px] text-gray-400">{item.pocPhone}</span>
                      )}
                    </td>
