...
export default function AdminMangalDalPage({ type }: Props) {
  ...
  // Apply block + expiry filter client-side
  const dals = useMemo(() => {
    const now = new Date();
    return allDals.filter((d: any) => {
  ...
  }, [allDals, filterBlockId, filterExpired]);
    ...
    <div className="p-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-4 flex flex-wrap gap-4 items-center shadow-sm">
        ...
        {(filterDistrictId || filterBlockId || filterExpired !== 'all') && (
          <button
            onClick={() => { setFilterDistrictId(''); setFilterBlockId(''); setFilterExpired('all'); }}
